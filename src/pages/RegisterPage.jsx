...
export default function RegisterPage() {
  ...
  return (
    <div style={styles.wrap}>
      ...
      <p style={styles.sub}>نیا اکاؤنٹ بنائیں اور 5000 کوائنز پائیں</p>

      <form onSubmit={handleSubmit} style={styles.card}>
        <label style={styles.label}>یوزرنیم</label>
        <input
          style={styles.input}
          value={username}
          onChange={(e) => setUsername(e.target.value)}
          placeholder="مثلاً: ali_khan"
          required
          minLength={3}
        />
        <label style={styles.label}>ای میل</label>
        <input
          style={styles.input}
          type="email"
          value={email}
        ...
        <button style={styles.btn} disabled={busy} type="submit">
          {busy ? 'انتظار کریں...' : 'اکاؤنٹ بنائیں'}
        </button>
      </form>

      <p style={styles.footer}>
        پہلے سے اکاؤنٹ ہے؟{' '}
        <Link to="/login" style={styles.link}>لاگ ان کریں</Link>
      </p>
    </div>
  );
}
